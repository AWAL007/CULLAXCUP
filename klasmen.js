...
const SoccerStandings = () => {
  ...
  const EditableCell = ({ row, field }) => {
    ...
    return (
      <td 
        className={`px-4 py-2 text-center ${field !== 'team' && field !== 'goal_difference' ? 'cursor-pointer hover:bg-gray-100' : ''}`}
  ...
  return (
    <div className="w-full max-w-4xl p-4">
      ...
      <p className="text-sm text-gray-600 mb-4">Klik pada angka untuk mengedit. Tabel akan otomatis diurutkan berdasarkan poin tertinggi.</p>
          ...
          <tbody className="divide-y divide-gray-200">
            {data.map((row, index) => (
              <tr key={row.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                ...
                <EditableCell row={row} field="goals_again
